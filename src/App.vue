...
<template>
	<main>
		...
		<div class="myanime" v-if="my_anime.length > 0">
			...
			<div v-for="anime in my_anime_asc" :key="anime.title" class="anime">
				<img :src="anime.image" />
				<h3>{{ anime.title }}</h3>
				...
				<!-- Zibil Qutusu -->
				<button @click="removeAnime(anime)" class="button delete-btn">🗑️</button>
			</div>
...
.button {
	...
	display: block;
	padding: 0.5rem 1rem;
	background-image: linear-gradient(to right, rgb(24, 138, 186) 50%, rgb(214, 38, 7) 50%);
	background-size: 200%;
	color: white;
	font-size: 1.125rem;
	font-weight: bold;
...
/* Zibil qutusu düyməsi üçün xüsusi stil */
.delete-btn {
	background-image: none;
	background-color: rgb(26, 228, 123);
	color: white;
	font-size: 1rem;
...
.delete-btn:hover {
	background-color: rgb(224, 52, 22);
}
